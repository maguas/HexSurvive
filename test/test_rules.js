...
try {
    ...
    game.resources = { scrap: 0, fuel: 10, food: 10, alloy: 0, intel: 0 };
    ...
    let gains = game.harvest(6);
    if (gains.scrap !== 1) throw new Error(`Should produce 1 Scrap, got ${gains.scrap}`);
    ...
    gains = game.harvest(8);
    if (gains.fuel !== 1) throw new Error(`Should produce 1 Fuel (Hero Adjacent), got ${gains.fuel}`);
    if (gains.food !== 0) throw new Error(`Should produce 0 Food (Not Adjacent), got ${gains.food}`);
    ...
    const startFuel = game.resources.fuel;
    const moveRes1 = game.moveHero(0, 0, 1);
    if (!moveRes1.success) throw new Error("Move failed");
    if (game.resources.fuel !== startFuel) throw new Error(`Move on Outpost should be free. Fuel: ${startFuel} -> ${game.resources.fuel}`);
