...
### Hex Map Tiles
- **Quantity**: 37 hex tiles
- **Size**: 2.5 inches (63.5mm) edge-to-edge per hex
...
- **Ruins** (Scrap): 7 tiles - Numbers: 3, 4, 5, 6, 8, 9, 10
- **Wasteland** (Fuel): 7 tiles - Numbers: 3, 4, 5, 6, 8, 9, 10
...
### Hero Figures (4 total - 1 per player)
- **Size**: ~25mm base, 30-35mm height
- **Design**: Distinct commander miniatures
- **Color-coded bases**: Red, Blue, Green, Yellow
- **Material**: Plastic injection molding or resin
...
- **Material**: Plastic injection molding
...
- **Color-coded**: Red, Blue, Green, Yellow (matching player colors)
- **Material**: Plastic injection molding
- **Base**: Circular base to fit on hex tile (replaces outpost when upgraded)
- **Visual**: Should clearly look more impressive than outposts
...
### Production Dice (2 standard d6)
- **Quantity**: 2 six-sided dice
- **Size**: Standard 16mm
...
- **Card Layout** (One Deck Dungeon style):
  - **Left side (~30% of card)**: Equipment/Loot information
    ...
    - Item name
  - **Right side (~70% of card)**: Encounter information
    - Encounter name and level
    - Enemy artwork
    - Defense slots (visual icons)
    - Slot requirements (Any/Tactics/Strength/Tech)
    - Resource rewards (optional)
    - Flavor text
- **Usage**: When defeated, card is tucked under Hero card with left edge visible showing equipment bonuses
- **Progression**: Hero power increases through collected equipment, not XP

### Hero/Commander Cards (4 cards)
- **Size**: Tarot/jumbo size (3.5" x 5.5") OR standard poker size
- **Material**: 300gsm card stock
- **Design**: Unique hero artwork per player
- **Information**:
  - Hero name and backstory
  - Starting stats
  - Special abilities (if implemented)
...
### Game Box
- **Size**: Standard large board game box (~12" x 12" x 3")
- **Material**: Sturdy cardboard with magnetic closure (deluxe) or standard lid
- **Box art**: Thematic artwork showing heroes vs aliens
- **Weight**: ~3-4 lbs (1.4-1.8 kg) estimated

### Plastic Insert/Organizer (optional)
- **Custom tray system** for:
  - Miniature storage (compartments per player color)
  - Card deck storage (sleeved or unsleeved)
  - Resource token compartments (5 sections)
  - Dice storage
  - Tile storage
- **Alternative**: Ziplock bags (budget option)
...
### Rulebook
- **Size**: ~8.5" x 11" or A4 format
- **Pages**: 16-24 pages estimated
- **Material**: Full-color glossy print
- **Content**:
  - Setup instructions with photos
  - Turn structure breakdown
  - Combat system explanation
  - Building and resource rules
  - Examples of play
  - FAQ section
  - Component list

### Quick Start Guide (optional)
- **Size**: Single double-sided sheet
- **Material**: Cardstock
- **Content**: Condensed rules for faster learning

---

## Summary: Total Piece Count
...
### 3D Miniatures Priority
Only heroes, outposts, and fortresses are 3D pieces to:
- Keep manufacturing costs reasonable
- Focus premium feel on player interaction pieces
- Make gameplay visually clear and tactile
- Simplified from original design (removed patrol figures)

### Dual-Purpose Encounter Cards (One Deck Dungeon System)
- **Design Philosophy**: Encounter cards serve double duty as equipment
- **Left edge (~30%)**: Equipment stats and bonuses
- **Right portion (~70%)**: Enemy encounter details
- **After victory**: Tuck card under Hero card, showing only equipment portion
- **Hero Progression**: Heroes grow stronger ONLY through collected equipment (no XP system)
- **Benefits**:
  - Eliminates need for 30-40 separate equipment cards
  - Reduces manufacturing cost by ~$2-3 per unit
  - Streamlines gameplay (defeat enemy = gain loot in one action)
  - Creates visual hero progression (cards stacked under hero)
